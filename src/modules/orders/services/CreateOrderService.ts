...
class CreateOrderService {
  ...
  public async execute({
    ...
    if (!customerExists) {
      throw new AppError('Não foi possível encontrar nenhum cliente com o ID fornecido.');
    }

    const existsProducts = await this.productsRepository.findAllByIds(products);

    if (!existsProducts.length) {
      throw new AppError('Não foi possível encontrar nenhum produto com os ids fornecidos.');
    }
